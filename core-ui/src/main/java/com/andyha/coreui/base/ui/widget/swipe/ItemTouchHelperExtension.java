package com.lucas.coreui.base.ui.widget.swipe;
...
public class ItemTouchHelperExtension extends RecyclerView.ItemDecoration
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                if (mSelected == null) {
                    ...
                    if (animation != null) {
                        ...
                        if (mSelected != null && mSelected.itemView instanceof ViewGroup) {
                            mCurrentView = findConsumeView((ViewGroup) mSelected.itemView, event.getRawX(), event.getRawY());
                            if (mCurrentView != null) {
                                mCallback.onItemTouch(event, mCurrentView, mSelected);
                            }
                        }
                    }
                }

                View clickView = mRecyclerView.findChildViewUnder(event.getX(), event.getY());
                int position = clickView != null ? mRecyclerView.getChildAdapterPosition(clickView) : -1;
                if (mPreOpened != null && position != -1 && mPreOpened.getAdapterPosition() != position) {
                    closeOpened();
                }
                ...
                if (mClick && action == MotionEvent.ACTION_UP) {
                    doChildClickEvent(event.getRawX(), event.getRawY(), mCurrentView);
                }
                if (mCurrentView != null && mCallback != null) {
                    mCallback.onItemTouch(event, mCurrentView, null);
                }
    ...
    public void closeOpened(boolean animate) {
        closeOpenedPreItem(animate);
    }
    ...
    private void closeOpenedPreItem(boolean animate) {
        View view = mCallback.getItemFrontView(mPreOpened);
        if (mPreOpened == null || view == null) return;
        if (mPreOpened instanceof BaseSwipeViewHolder) {
            view = ((BaseSwipeViewHolder) mPreOpened).getContainerLayout();
        }

        if (mPreOpened instanceof BaseSwipeViewHolder) {
            ((BaseSwipeViewHolder) mPreOpened).setCurDir(0);
        }
        ViewHolder closeHolder = mPreOpened;

        float currentX = view.getTranslationX();
        if (Math.abs(currentX) < 30) return;
        ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(view, "translationX", currentX, 0f);
        objectAnimator.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationStart(Animator animation) {
                super.onAnimationStart(animation);
                if (mPreOpened != null) mCallback.clearView(mRecyclerView, mPreOpened, true);
        ...
        objectAnimator.addUpdateListener(animation -> {
            if (closeHolder instanceof BaseSwipeViewHolder) {
                ((BaseSwipeViewHolder) closeHolder).progress((Float) animation.getAnimatedValue());
            }
        });
        objectAnimator.setDuration(animate ? 300L : 0L);
        objectAnimator.start();
    }

    private void doChildClickEvent(float x, float y, View currentView) {
        if (mSelected == null) return;

        View consumeEventView = mSelected.itemView;

        if (mSelected instanceof BaseSwipeViewHolder) {
            int preDir = swipeIfNecessary(mSelected);
            if (preDir == 0) return;
        }
        ...
        if (currentView != null && currentView == consumeEventView && currentView.isEnabled() && SwipeOptionLayout.isOptionView(currentView)) {
            if (consumeEventView.hasOnClickListeners()) {
    ...
    private void select(ViewHolder selected, int actionState) {
        ...
        if (mSelected != null) {
            ...
            if (prevSelected.itemView.getParent() != null) {
                ...
                if (prevSelected instanceof BaseSwipeViewHolder) {
                    ((BaseSwipeViewHolder) prevSelected).setState(swipeDir);
                }
                releaseVelocityTracker();
                ...
                final RecoverAnimation rv = new RecoverAnimation(prevSelected, animationType,
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        if (swipeDir <= 0) {
                            ...
                            mCallback.clearView(mRecyclerView, prevSelected, true);
                        } else {
    ...
    private float getSwipeWidth() {
        if (mSelected instanceof BaseSwipeViewHolder) {
            return ((BaseSwipeViewHolder) mSelected).getActionWidth();
        }
            ...
            public void run() {
                if (mRecyclerView != null && mRecyclerView.isAttachedToWindow() &&
                    ...
                    if ((animator == null || !animator.isRunning(null))
                        ...
                        if (anim.mViewHolder instanceof BaseSwipeViewHolder) {
                            ((BaseSwipeViewHolder) anim.mViewHolder).setCurDir(swipeDir);
                        }
                    } else {
    ...
    public ViewHolder findSwipedView(MotionEvent motionEvent) {
        final RecyclerView.LayoutManager lm = mRecyclerView.getLayoutManager();
        if (mActivePointerId == ACTIVE_POINTER_ID_NONE) {
            return null;
        }
    ...
    public abstract static class Callback {
        ...
        public void clearView(RecyclerView recyclerView, ViewHolder viewHolder) {
            clearView(recyclerView, viewHolder, false);
        }

        public void clearView(RecyclerView recyclerView, ViewHolder viewHolder, boolean close) {
        ...
        public abstract void onItemTouch(MotionEvent event, View view, ViewHolder viewHolder);
